...
const TeacherMessages = () => {
  ...
  const parentLinkIdRef = useRef(null);
  ...
  const openStudentChat = async (student) => {
    ...
    parentLinkIdRef.current = null;

    // Clear any existing poll
    if (pollRef.current) clearInterval(pollRef.current);

    try {
      // Get the student's parent link
      const studentId = student.student?.id || student.student;
      const statusRes = await fetch(`${baseUrl}/student/${studentId}/parent/status/`);
      const statusData = await statusRes.json();
      
      if (statusData.parent_link_id) {
        const linkId = statusData.parent_link_id;
        setParentLinkId(linkId);
        parentLinkIdRef.current = linkId;
        ...
        setChatStatus({ allowed: false, reason: 'No parent account linked to this student' });
      }
    } catch (err) {
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 60px)', overflow: 'hidden' }}>
      ...
      <div style={{ width: isMobile && activeStudent ? '0' : (isMobile ? '100%' : '280px'), borderRight: '1px solid #e2e8f0', backgroundColor: '#fff', overflow: 'auto', transition: 'width 0.3s', flexShrink: 0 }}>
        <div style={{ padding: '16px', borderBottom: '1px solid #e2e8f0' }}>
          ...
          <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: '12px' }}>Chat with student parents</p>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: '#f8fafc' }}>
        {!activeStudent ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
            ...
            <p style={{ color: '#64748b', marginTop: '12px' }}>Select a student to start messaging their parent</p>
          </div>
        ) : (
          <>
            {/* Header */}
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {messages.length === 0 ? (
                <div style={{ textAlign: 'center', color: '#94a3b8', padding: '40px', fontSize: '14px' }}>
                  {parentLinkId ? 'No messages yet' : 'Parent account not linked'}
                </div>
              ) : (
                messages.map(msg => {
