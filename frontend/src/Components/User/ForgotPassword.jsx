...
const ForgotPassword = () => {
  ...
  useEffect(() => {
    document.title = 'LMS | Student Forgot Password';
    window.scrollTo(0, 0);
  }, []);
  ...
  const requestReset = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/password-reset/request/student/`, formData);
  ...
  const submitNewPassword = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/password-reset/confirm/student/`, formData);
      if (res.data.bool) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f5f7fa 0%, #e8ecf1 100%)', padding: '40px 20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: '480px', background: 'white', borderRadius: '16px', padding: '40px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03)' }}>
        <Link to="/student/login" style={{ color: '#6b7280', textDecoration: 'none', fontSize: '14px' }}>← Back to login</Link>
        ...
        {!token ? (
          <>
            <input
              ...
              placeholder="student@kannari.com"
